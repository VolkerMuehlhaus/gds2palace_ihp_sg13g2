...
**palace_summary.py** prints a human-readable summary of AWS Palace solver results (degrees of freedom, mesh size, simulation time, peak RAM, and adaptive-mesh-refinement error indicators) for a completed or in-progress run. Run it with no arguments from inside the `<model>_data` directory that contains `config.json`, or pass that directory as an argument. The model base name is auto-derived from the directory name (stripping the trailing `_data`); use `--model-basename` to override this if needed. Exits with a nonzero status if no results are found yet, so it can be chained after `run_palace` in a script to detect whether the simulation actually finished.
